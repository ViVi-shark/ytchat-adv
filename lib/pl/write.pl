...
sub diceCodeCheck {
  ...
  if($::in{'comm'} =~ /\s((?:
    ...
    | 威力 | [rk][ァ-ヴ] | 成長
    | ([a-zａ-ｚA-ZＡ-Ｚ0-9０-９\+＋\-－\*＊\/／()（）]*dc[a-zａ-ｚA-ZＡ-Ｚ0-9０-９\+＋\-－\*＊\/／()（）]*(\s*(add|sub)\s*[a-zａ-ｚA-ZＡ-Ｚ0-9０-９\+＋\-－\*＊\/／()（）]+)?)
  )(?!.*\s).*)$/ix){
    require './lib/pl/dice.pl';
    my $code;
  ...
  if (defined($diceCodeRow)) {
    ...
    foreach my $row (@rowSources) {
      ...
      if ($row =~ s/^(\d+(?:-(?:\d+)?)?|-\d+)\s+//) {
        ...
        if (defined($begin) && $begin ne '') {
          foreach my $i ($begin .. (defined($end) && $end ne '' ? $end : $begin)) {
            push(@rows, "$i:$row");
          }
        }
      }
