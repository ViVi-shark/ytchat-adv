...
function boxOpen(id){
  ...
  if (box == null) {
    console.warn(`Box '${id}' is not found.`);
    return null;
  }
...
function topicOpen(){
  ...
  if (currentValue == null || currentValue === '') {
    currentValue = $('#topic-value').text();
  }

  document.getElementById('edit-topic-value').value = currentValue ?? '';
}
// 名前欄 ----------------------------------------
// 一括編集を開く
function npcOpen(){
...
function setUnitLink(unitName){
  if (unitList[unitName]['url'] != null && unitList[unitName]['url'] != '') {
    const node = document.getElementById(`stt_unit_${unitList[unitName]['id']}_link`);
    if (node.getAttribute('href') == null || node.getAttribute('href') == '') {
      node.setAttribute('href', unitList[unitName]['url']);

      $.ajax(
        {
          type: 'GET',
          url: 'lib/php/get_image_url.php?sheet=' + encodeURI(unitList[unitName]['url']),
          dataType: "text",
          success: result => {
            ...
            document.querySelector(`#stt-unit-${unitList[unitName]['id']}`).setAttribute('data-image-url', result);
            $('head').append($(`<style type="text/css">[id="stt-unit-${unitList[unitName]['id']}"]:before{ background-image: url(${result}); }</style>`));
          }
        }
      );
    }
...
function unitAdd(unitName){
  ...
  newUnit.innerHTML = '<dt onclick="sheetSelect(\''+unitId+'\');sheetOpen();" style="color:'+unitList[unitName]['color']+';"><span class="name">'+unitName+`</span><a class="reference" id="stt_unit_${unitId}_link" target="_blank"></a></dt>`;
  if(unitList[unitName]['status'] === undefined) { unitList[unitName]['status'] = {}; }
  newUnit.innerHTML += `<dd id="stt-memo-${unitId}"><span class="icon" /></dd>`;
  document.getElementById("status-body").appendChild(newUnit);
  $(`#stt_unit_${unitId}_link`).on(
      'click',
      e => e.stopPropagation()
  );

  makeTooptip(
    $(`#stt-memo-${unitId} > .icon`),
    (function($memoContainer){
      return () => $memoContainer.attr('data-memo');
    })($(`#stt-memo-${unitId}`))
  );
...
function memoUpdate(){
  ...
  for (let i in shareMemo) {
    ...
    const memoStructure = shareMemo[i] != null && shareMemo[i] != '' ? JSON.parse(decodeURIComponent(shareMemo[i])) : [];
    const [_, memoHtml] = memoStructure;

    newMemo.innerHTML = `<div class="headline">${memoHtml != null ? $('<div />').html(memoHtml.split(/(\n|<(br|hr)\s*\/?>|<\/.+?>)/)[0]).text() : ''}</div>`;
    newMemo.setAttribute("onclick","memoSelect("+i+");");
    newMemo.setAttribute('data-memo-index', (`0${parseInt(i) + 1}`).substr(-2));
    document.getElementById("memo-list").appendChild(newMemo);

    const $buttonToOpen = $('<button class="to-open" />').text('🔎');
...
// 選択
let selectedMemo = '';
function memoSelect(num, mode){
  const memoStructure = shareMemo[num] != null ? JSON.parse(decodeURIComponent(shareMemo[num])) : [];
  const [memoSource, memoHtml] = memoStructure;

  const memoValue = document.getElementById("sheet-memo-value");
  memoValue.value = memoSource ? memoSource : '';
  document.getElementById("sheet-memo-pretty").innerHTML = memoHtml != null ? memoHtml : '';

  if (mode != null) {
    $('.sheet-memo-body').attr('data-mode', mode);
  } else {
    $('.sheet-memo-body').attr('data-mode', 'view');
  }
  ...
  $('#sheet-memo-value').focus();
}

(function($){
  const $container = $('.sheet-memo-body');

  $container.find('.memo-tab[data-mode-name="view"]').on(
    'click',
    () => $container.attr('data-mode', 'view')
  );
  $container.find('.memo-tab[data-mode-name="edit"]').on(
    'click',
    () => $container.attr('data-mode', 'edit')
  );

  const $view = $('#sheet-memo-pretty');
  $view.on(
    'click',
    () =>
    {
...
function tabGroupAdd(){
  ...
  let setFontSize = fontSize[groupNum] || 100;
  ...
  document.getElementById("chat-area").appendChild(newGroup);
...
function tabAdd(tabNum){
  ...
  newTab.innerHTML = `
    ...
    `;
  let targetGroup; let maxGroup = 3;
  if(window.matchMedia('(max-width:600px)').matches){ maxGroup = 1 }
  for (let i = 1; i <= maxGroup; i++) {
    if(!targetGroup && !document.getElementById("tab-group"+i)){
      targetGroup = tabGroupAdd();
    }
  }
...
function bgmHistoryUpdate(){
  ...
  for (let i in sortHistory){
    ...
    $(`#bgm-preset .track[data-track-full-name="${title}"]`).addClass('exists-in-history');
  }
}

(function ($) {
  function playSe(soundPath, ignoreMute = false) {
    if (muteOn && !ignoreMute) {
      return;
    }

    const audio = new Audio(`lib/sound/${soundPath}`);

    audio.volume = (volumes['chat'] / 100) * (volumes['master'] / 100);
    audio.currentTime = 0;
    audio.play();
  }

  /** @var {Array<{path: string, node: jQuery}>} */
  const trackList = [];

  const $list = $('#sound-effect-window .sound-effect-list');

  $list.find('.sound-effect').each(
      function (_, node) {
        const $node = $(node);
        const $buttonToTrial = $node.find('button.to-trial');
        const $buttonToInertToChat = $node.find('button.to-insert-to-chat');
        ...
        $buttonToInertToChat.on(
            'click',
            function () {
              const $chatTextArea = $('#form-comm-main');
              $chatTextArea[0].value += `[<se:${path}>]`;
            }
        );

        trackList.push({path: path, node: $node});
      }
  );

  {
    const $filterForm = $('#sound-effect-window .filter-form');

    const $filterTextInput = $filterForm.find('input[type="text"]');
    const $buttonToClearFilter = $filterForm.find('button.to-clear');

    $filterTextInput.on(
        'change input',
        () => {
          const conditionText = $filterTextInput.val();

          const classToHide = 'hidden';
          const classToEnableFilter = 'filter-enabled';
          const classToMatched = 'matched-by-filter';
          let operation;
          if (conditionText === '' || /^\s+$/.test(conditionText)) {
            $buttonToClearFilter.addClass(classToHide);
            operation = (_, $node) => $node.removeClass(classToMatched);
            $list.removeClass(classToEnableFilter);
          } else {
            $buttonToClearFilter.removeClass(classToHide);
            operation = (path, $node) => {
              if (path.includes(conditionText)) {
                $node.addClass(classToMatched);
              } else {
                $node.removeClass(classToMatched);
              }
            };
            $list.addClass(classToEnableFilter);
          }

          for (const track of trackList) {
            operation(track.path, track.node);
          }
        }
    ).trigger('change');

    $buttonToClearFilter.on(
        'click',
...
function seMute(mute) {
  for (let i = 0; i < lastSoundEffectList.length; i++) {
    lastSoundEffectList[i].muted = mute;
  }
}
...
function makeTooptip($node, textSelector){
  $(
    function(){
      const $tooltipInner = $('<div />');
      const $tooltip = $('<div class="tooltip hidden">').append($tooltipInner);
      ...
      $node.on(
        'mouseenter',
        function(){
          $tooltipInner.text(textSelector($node));
          $tooltip.css('left', ($node.offset().left + $node.width()) + 'px');
          $tooltip.css('top', ($node.offset().top + $node.height() / 2) + 'px');
