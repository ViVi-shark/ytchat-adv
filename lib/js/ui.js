...
function resolveGoogleDriveAssetUrl(url) {
  if (
      url.match(/^https?:\/\/drive\.google\.com\/file\/d\/(.+)\/view\?usp=(?:sharing|(?:share|drive)_link)$/) ||
      url.match(/^https?:\/\/drive\.google\.com\/uc\?(?:.+&)?id=(.+?)(?:$|&)/)
  ) {
    return `${location.origin}${location.pathname}lib/php/download_asset.php?url=${url}`;
  }
...
function oldLogHide(tabId){
  ...
  let needToRefreshHeadlineNavigator = false;
  for (let i = count; i > 40; i--) {
    needToRefreshHeadlineNavigator |= tab.firstElementChild.querySelector('h1, h2, h3, h4, h5, h6') != null;
    oldLogs[tabId].push(tab.firstElementChild);
    tab.removeChild(tab.firstElementChild);
  }
  if (needToRefreshHeadlineNavigator) {
...
function bottomCheck(tabId,obj) {
  ...
  //最上部までスクロールしたら古いログを表示
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if(!insert) break;
      obj.prepend(insert);
      needToRefreshHeadlineNavigator |= insert.querySelector('h1, h2, h3, h4, h5, h6') != null;
    }
...
function boxOpen(id,details){
  ...
  if(details){
    ...
    if(boxObj.classList.contains('open') && detailsObj.open){
      ...
      if (!boxObj.classList.contains('open')) {
        boxObj.dispatchEvent(new Event('opening'));
      }
...
function nameChange(num){
  ...
  {
    const pictureUnitSelector = document.querySelector('#main-form .comm-config-area .picture-area .unit-select-area select');
    const targetOption = pictureUnitSelector.querySelector(`option[data-unit-name="${nameList[num]['name']}"]`);
    if (targetOption != null) {
      pictureUnitSelector.value = targetOption.getAttribute('value');
    } else {
      pictureUnitSelector.selectedIndex = 0;
    }
    pictureUnitSelector.dispatchEvent(new Event('change'));
  }
...
function diceAdd(area,re){
  ...
  const formDice = document.querySelector("#main-form #form-dice-"+area);
  if (formDice != null) {
    formDice.appendChild(newOutline);
  }
  ...
  const diceButton = document.getElementById('dice-button-'+area+'-'+n);
  if (diceButton != null) {
    autosize(diceButton);
  }
  saveRoomConfig();
...
function unitRemoconAdd(unitName,status,value){
  ...
  nameInput.addEventListener(
      ...
      event => {
        ...
        container.querySelector('.label').textContent = statusName;
      }
  );
...
function setUnitLink(unitName) {
  ...
  const unitId = unitList[unitName]['id'];
  const unitLinkedUrl = unitList[unitName]['url'];
  if (unitId == null || unitId === '' || unitLinkedUrl == null || unitLinkedUrl === '') {
    return;
  }

  const linkContainerNode = document.getElementById(`stt-url-${unitId}`);
  if (linkContainerNode == null) {
    return;
  }

  if (linkContainerNode.querySelector('a') == null) {
    const linkNode = document.createElement('a');
    linkNode.setAttribute('href', unitLinkedUrl);
    linkNode.setAttribute('target', '_blank');

    linkContainerNode.append(linkNode);
  }

  fetch('lib/php/get_image_url.php?sheet=' + encodeURI(unitLinkedUrl))
      .then(x => x.text())
      .then(result => {
        if (result == null || result === '' || !result.match(/^(https?:\/\/|data:)/)) {
          return;
        }

        document.querySelector(`#stt-unit-${unitId}`).setAttribute('data-image-url', result);
        const styleNode = document.createElement('style');
        styleNode.setAttribute('type', 'text/css');
        styleNode.textContent = `[id="stt-unit-${unitId}"]:before{ background-image: url(${result}); }`;
        document.querySelector('head').append(styleNode);

        {
          const sheet = document.getElementById(`sheet-unit-${unitId}`);
          if (sheet != null && sheet.querySelector(`.message-picture-area > .picture-list > [data-label="デフォルト"][data-picture-url="${result}"]`) == null) {
            const node = document.createElement('li');
            node.classList.add('default');
            node.dataset.label = node.textContent = 'デフォルト';
            node.dataset.pictureUrl = result;

            sheet.querySelector('.message-picture-area > .picture-list').prepend(node);

            refreshUnitPictureSelector(unitId);
          }
        }

        document.querySelectorAll('.chat .logs dl > dt.picture:not(.resolved)').forEach(
            node => {
              const talkerName = node.parentNode.querySelector('dt[id$="-name"]').textContent.trim();

              if (talkerName === unitName) {
                resolveMessagePicture(node, null, unitName);
              }
            }
        );
      });
...
function unitAdd(unitName){
  ...
  details.innerHTML = `<ul class="details"><li id="stt-memo-${unitId}"><span class="material-symbols-outlined icon">description</span></li></ul>`;
  newUnit.appendChild(details);
  newUnit.innerHTML += `<dd id="stt-url-${unitId}"></dd>`;
  document.getElementById("status-body").appendChild(newUnit);

  setUnitLink(unitName);

  makeTooltip(
    document.getElementById(`stt-memo-${unitId}`),
    memoContainerElement => memoContainerElement.getAttribute('data-memo')
  );
...
ダブルクリックで即送信`;
  ...
  if(selectedSheet !== unitId){ newSheet.classList.add('hidden'); }
  ...
  newSheet.innerHTML = `
      ...
      <div class="sheet-body">
        ...
        <div class="message-picture-area">
          <h3>発言者画像</h3>
          <span class="add button">＋</span>
          <span class="del button">－</span>
          <ul class="picture-list"></ul>
        </div>
  ...
  {
    const quickModeCheckbox = newSheet.querySelector('.status-remocon-area .mode-switcher input[type="checkbox"]');

    quickModeCheckbox.addEventListener(
        'input',
        () => {
          const isSimpleMode = quickModeCheckbox.checked;
          quickModeCheckbox.closest('.status-remocon-area').classList.toggle('quick-mode', isSimpleMode);
        }
    );
  }
  ...
  
  {
    ...
    option.setAttribute('value', unitId);
    option.dataset.unitId = unitId;
    option.dataset.unitName = option.textContent = unitName;

    const selector = document.getElementById('message-picture-selector');
    selector.querySelector(`option[data-unit-id="${unitId}"]`)?.remove();
  ...
  
  {
    ...
    document.querySelector('.comm-config-area .picture-select-area').appendChild(label);
  }

  {
    const messagePictureArea = newSheet.querySelector('.message-picture-area');
    const pictureList = messagePictureArea.querySelector('.picture-list');

    messagePictureArea.querySelector('.button.add').addEventListener(
        'click',
        () => {
          const pictureId = randomId(12);

          addPictureSettings(unitName, pictureId);

          refreshUnitPictureSelector(unitId);
          saveUnitPictureSettings(unitId);
        }
    );

    messagePictureArea.querySelector('.button.del').addEventListener(
        'click',
        () => {
          const lastItem = pictureList.querySelector('li:not(.default)');
  ...
  document.querySelector('body').dispatchEvent(new Event('unit-added'));
}
...
function sheetOpen(){
  if (selectedSheet !== 'default' && selectedSheet !== 'memo' && romMode) {
    sheetClose();
    return;
  }
  ...
  document.getElementById('sheet-area').dispatchEvent(new Event('sheet-open'));
}
function sheetClose(){
  document.getElementById('sheet-area').classList.add('closed');
  sheetOpenCheck = 0;

  document.getElementById('sheet-area').dispatchEvent(new Event('sheet-close'));
}
function sheetSelect(id){
  ...
  document.getElementById('sheet-unit-'+id).classList.remove('hidden');
  selectedSheet = id;
...
// ユニットのシートを切り替えるショートカット用 ----------------------------------------
function sheetSelectPreviousUnit() {
  sheetSelectSiblingUnit(-1);
}
function sheetSelectNextUnit() {
  sheetSelectSiblingUnit(+1);
}
/**
 * @param {int} offset
 */
function sheetSelectSiblingUnit(offset) {
  const selectedUnitName =
      document.querySelector('#sheet-area > [id^="sheet-unit-"]:not(.hidden)')
          ?.querySelector('.chara-name')
          ?.textContent.trim();
  ...
  const selectedUnitElement =
      document.querySelector(`#status-body > [data-name="${selectedUnitName}"]`);

  if (selectedUnitElement == null) {
    return;
  }

  let element = selectedUnitElement;
  let unresolvedOffset = Math.round(offset);

  while (unresolvedOffset !== 0) {
    if (unresolvedOffset > 0) {
      element = element.nextElementSibling ?? element.parentElement.firstElementChild;
      unresolvedOffset--;
    } else {
      element = element.previousElementSibling ?? element.parentElement.lastElementChild;
      unresolvedOffset++;
    }
  }

  const m = element.id.match(/^stt-unit-(.+?)$/);
  const targetUnitId = m[1];

  sheetSelect(targetUnitId);
}
// チャットパレット編集 ----------------------------------------
...
function paletteSet(id,textData){ // テキストからセレクトボックスへセット
  ...
  for(let i=0; i<lines.length; i++){
    if(lines[i].match(/^###(.+)$/)){
      ...
      op.innerHTML = tagConvert(text, {noStandardTooltip: type === 'param'});
      op.dataset.value = lines[i].replace(/\s+;.+$/, '');
      op.tabIndex = '0';
...
// 発言者画像 ----------------------------------------
function addPictureSettings(unitName, pictureId, pictureName = null, pictureUrl = null) {
  ...
  const pictureList = sheet.querySelector('.message-picture-area .picture-list');

  let pictureListItem = pictureList.querySelector(`li[data-id="${pictureId}"]`);

  if (pictureListItem == null) {
    pictureListItem = document.createElement('li');
    pictureListItem.dataset.id = pictureId;

    {
      const label = document.createElement('label');
      label.classList.add('picture-name');
      label.textContent = '識別名';

      const input = document.createElement('input');
      input.setAttribute('type', 'text');
      input.addEventListener('input', () => refreshUnitPictureSelector(unitId, pictureId));
      input.addEventListener('change', () => refreshUnitPictureSelector(unitId, pictureId));
      input.addEventListener('blur', () => saveUnitPictureSettings(unitId));

      label.appendChild(input);
      pictureListItem.appendChild(label);
    }

    {
      const label = document.createElement('label');
      label.classList.add('picture-url');
      label.textContent = 'ＵＲＬ';
  ...
  if (pictureName != null) {
    const input = pictureListItem.querySelector('label.picture-name > input');
    input.value = pictureName;
    input.dispatchEvent(new Event('change'));
  }

  if (pictureUrl != null) {
    const input = pictureListItem.querySelector('label.picture-url > input');
    input.value = pictureUrl;
    input.dispatchEvent(new Event('change'));
  }
}
function refreshUnitPictureSelector(unitId, pictureId = null) {
  const unitPictureSelector = document.querySelector(`.comm-config-area .picture-area .picture-select-area label[data-unit-id="${unitId}"] select`);
  const sheet = document.getElementById(`sheet-unit-${unitId}`);

  if (unitPictureSelector == null || sheet == null) {
    return;
  }

  if (pictureId != null) {
    const option = unitPictureSelector.querySelector(`option[data-picture-id="${pictureId}"]`);
    if (option == null) {
      return;
    }

    const pictureSettingsNode = sheet.querySelector(`.picture-list > [data-id="${pictureId}"]`);
    if (pictureSettingsNode == null) {
      return;
    }

    const pictureName = pictureSettingsNode.querySelector('label.picture-name > input').value;
    const pictureUrl = pictureSettingsNode.querySelector('label.picture-url > input').value;

    option.textContent = pictureName;
    option.value = pictureUrl;

    return;
  }

  unitPictureSelector.innerHTML = '';

  sheet.querySelectorAll('.picture-list > *').forEach(
      pictureSettingsNode => {
        const option = document.createElement('option');
        option.dataset.pictureId = pictureSettingsNode.dataset.id ?? '';

        if (pictureSettingsNode.classList.contains('default')) {
          option.textContent = "デフォルト";
          option.value = pictureSettingsNode.dataset.pictureUrl;
        } else {
          const pictureName = pictureSettingsNode.querySelector('label.picture-name > input').value;
          const pictureUrl = pictureSettingsNode.querySelector('label.picture-url > input').value;
        ...
        unitPictureSelector.appendChild(option);
      }
  );
}
async function saveUnitPictureSettings(unitId) {
  const sheet = document.getElementById(`sheet-unit-${unitId}`);
  ...
  const unitName = sheet.querySelector('h2.chara-name').textContent.trim();
  const settings = [];

  sheet.querySelectorAll(`.picture-list > [data-id]`).forEach(
      node => {
        const id = node.dataset.id;
        const name = node.querySelector('label.picture-name > input').value;
        const url = node.querySelector('label.picture-url > input').value;

        settings.push({id: id, name: name, url: url});
      }
  );
  ...
  commSend(
      `/picture-settings-update ${base64}`,
      0,
      unitName,
      unitList[unitName]['color'] ?? ''
  );
}
{
  const pictureArea = document.querySelector('.comm-config-area .picture-area');

  const unitSelector = pictureArea.querySelector('.unit-select-area select');

  function onUnitSelected() {
    const selectedUnitId = unitSelector.value;

    pictureArea.querySelectorAll('.picture-select-area > label').forEach(
        node => node.classList.toggle('active', node.dataset.unitId === selectedUnitId)
    );
  }

  unitSelector.addEventListener('input', onUnitSelected);
  unitSelector.addEventListener('change', onUnitSelected);
}
...
// 選択
let selectedMemo = '';
function memoSelectAndZoom(num){
  memoSelect(num);

  const memoIndex = parseInt(num);
  const memoContainer = document.createElement('div');
  memoContainer.innerHTML = shareMemo[memoIndex] ? tagConvert(shareMemo[memoIndex]) : '';
  memoZoomModal.open(memoContainer);
}
...
function memoShowModal(num = null){
  ...
  readyModalDialogToClose(dialog);
}

/**
 * @param {HTMLDialogElement} dialog
 */
function readyModalDialogToClose(dialog) {
  dialog.addEventListener("click", function closeModal(e) {
...
function shrinkMemoList() {
  const sideBarNode = document.getElementById('sidebar');

  document.querySelector('#memo .button.shrink > .icon').textContent =
      sideBarNode.classList.toggle('memo-shrunken') ? 'expand_all' : 'collapse_all';
}
...
{
  /**
   * @param {HTMLElement} mouseEventTarget
   * @return {HTMLElement|null}
   */
  function findReferenceElement(mouseEventTarget) {
    return mouseEventTarget.classList.contains('unit-reference')
        ? mouseEventTarget
        : mouseEventTarget.closest('.unit-reference');
  }

  /**
   * @param {HTMLElement} unitReferenceElement
   * @return {string}
   */
  function getUnitNameByUnitReferenceElement(unitReferenceElement) {
    return unitReferenceElement.dataset.unitName;
  }

  /**
   * @param {string} unitName
   * @return {Array<{name: string, value: {current: int, max: int}}>}
   */
  function findStatusByUnitName(unitName) {
    const unitStatusElement = document.querySelector(`#status-body > [data-name="${unitName}"]`);

    if (unitStatusElement == null) {
      return [];
    }

    /** @var {Array<{name: string, value: {current: int, max: int}}>} */
    const statusItems = [];

    unitStatusElement.querySelectorAll('.details [data-stt*="HP"]:has(.value):has(.gauge)').forEach(
        statusElement => {
          const statusName = statusElement.dataset.stt;

          const valueElement = statusElement.querySelector('.value');
          const valueText = valueElement.textContent.trim();

          const [currentValue, maxValue] = valueText.split('/').map(x => parseInt(x));

          statusItems.push({name: statusName, value: {current: currentValue, max: maxValue}});
        }
    );
  ...
  /**
   * @param {string} unitName
   * @param {Array<{name: string, value: {current: int, max: int}}>} statusItems
   * @return {HTMLElement}
   */
  function createUnitStatusViewElement(unitName, statusItems) {
    /**
     * @param {string} unitName
     * @return {HTMLElement}
     */
    function createUnitNameElement(unitName) {
      const element = document.createElement('span');
      element.classList.add('unit-name');
     ...
     * @return {HTMLElement}
     */
    function createStatusListElement(statusItems) {
      /**
       * @param {{name: string, value: {current: int, max: int}}} item
      ...
      function createItemElement(item) {
         ...
         * @return {HTMLElement}
         */
        function createStatusNameElement(statusName) {
          const element = document.createElement('span');
          element.classList.add('status-name');
        ...
        /**
         * @param {{current: int, max: int}} value
         * @return {HTMLElement}
         */
        function createValueElement(value) {
           ...
           * @return {HTMLElement}
           */
          function createValueElement(className, value) {
            const element = document.createElement('span');
          ...
          valueElement.appendChild(createValueElement('current', value.current));
          valueElement.appendChild(createValueElement('max', value.max));
         ...
         * @return {HTMLElement}
         */
        function createGaugeElement(currentValue, maxValue) {
          const ratio = maxValue > 0 ? Math.max(0, Math.min(1, currentValue / maxValue)) : 0;

          const gaugeElement = document.createElement('span');
        ...
        li.dataset.statusName = item.name;
        li.appendChild(createStatusNameElement(item.name));
        li.appendChild(createValueElement(item.value));
        li.appendChild(createGaugeElement(item.value.current, item.value.max));
      ...
      for (const statusItem of statusItems) {
        ul.appendChild(createItemElement(statusItem));
      }

      return ul;
    }

    const unitStatusViewElement = document.createElement('div');
    unitStatusViewElement.classList.add('popup-unit-view');
    unitStatusViewElement.appendChild(createUnitNameElement(unitName));
    unitStatusViewElement.appendChild(createStatusListElement(statusItems));

    return unitStatusViewElement;
  }
   ...
   */
  function setupViewPosition(view, referenceElement) {
    const referenceElementRect = referenceElement.getBoundingClientRect();

    view.style.left = `${Math.floor(referenceElementRect.left)}px`;
    view.style.top = `${Math.floor(referenceElementRect.bottom) - 5}px`;

    {
      ...
      if (viewRect.right > windowRight) {
        viewRect.style.left = `${Math.floor(referenceElementRect.left - (viewRect.width - windowRight))}px`;
      }

      if (viewRect.bottom > windowBottom) {
        viewRect.style.top = `${Math.floor(referenceElementRect.top - viewRect.height + 5)}px`;
      }
    }
  }
   ...
   * @param {HTMLElement} referenceElement
   */
  function makeViewBehaviour(view, referenceElement) {
    const onMouseOut = /** @param {MouseEvent} event */event => {
      /**
      ...
      if (
          pointInRect(point, view.getBoundingClientRect()) ||
          pointInRect(point, referenceElement.getBoundingClientRect())
      ) {
        return;
      }
    ...
    view.addEventListener(
        ...
        () => {
          ...
          referenceElement.removeEventListener('mouseout', onMouseOut);
          view.remove();

          referenceElement.classList.remove('has-view');
        }
    );

    view.addEventListener('mouseout', onMouseOut);
    referenceElement.addEventListener('mouseout', onMouseOut);
  }
  ...
  document.querySelector('body').addEventListener(
      ...
      event => {
        ...
        const unitReferenceElement = findReferenceElement(eventTarget);

        if (unitReferenceElement == null || unitReferenceElement.classList.contains('has-view')) {
          return;
        }

        document.querySelectorAll('.popup-unit-view').forEach(
            x => x.dispatchEvent(new Event('close'))
        );

        const unitName = getUnitNameByUnitReferenceElement(unitReferenceElement);
        const statusItems = findStatusByUnitName(unitName);

        if (statusItems.length === 0) {
          return;
        }

        unitReferenceElement.classList.add('has-view');

        const unitStatusViewElement = createUnitStatusViewElement(unitName, statusItems);
        document.querySelector('body').appendChild(unitStatusViewElement);

        setupViewPosition(unitStatusViewElement, unitReferenceElement);
        makeViewBehaviour(unitStatusViewElement, unitReferenceElement);
      }
  );
}
...
document.querySelectorAll('.insert-quote').forEach(obj => { obj.addEventListener('click', (e)=>{
  tagInsert(e, '<quoted>','</quoted>')
}); });
...
function bgmSet(){
  if(beforeBGM != currentBgm['url']){
    if(currentBgm['url'].match(/^https:\/\/youtu\.be\/(.+)$/)){
      ...
      bgMusic.src = resolveCloudAssetUrl(currentBgm['url']);
      bgMusic.play();
    }
...
function bgmHistoryUpdate(){
  ...
  for (let i in sortHistory){
    ...
    function escape(text) {
      ...
      return dummy.innerHTML.replaceAll('"', '&quot;');
    }
    document.querySelectorAll(`#bgm-preset .track[data-track-full-name="${escape(title)}"]`).forEach(
        node => node.classList.add('exists-in-history')
    );
  }
}
// 背景 ----------------------------------------
...
function bgInputSet(url, title, mode = null){
  document.getElementById('bg-set-preview').src = resolveCloudAssetUrl(url);
...
document.onkeydown = function(e){
  if ((e.ctrlKey && !e.metaKey) || (!e.ctrlKey && e.metaKey)) {
    ...
    else if (e.keyCode === 13 && e.target.getAttribute('id') === 'rewrite-comm') {
      e.preventDefault();
      e.target.closest('.comm-area').querySelector('button').dispatchEvent(new Event('click'));
      return;
    }
  }
  // フォーム上のキー動作
...
function makeTooltip(node, textSelector) {
  const classNameOfInner = 'inner';
  const classNameToHide = 'hidden';

  function createTooltipNode() {
    const innerElement = document.createElement('div');
    innerElement.classList.add(classNameOfInner);

    const outerElement = document.createElement('div');
    outerElement.classList.add('old-tooltip', classNameToHide);
    outerElement.append(innerElement);

    return outerElement;
  }

  function updateTooltipNode(tooltipNode, textSelector, leftPosition, topPosition) {
    tooltipNode.querySelector(`.${classNameOfInner}`).textContent = textSelector(node) ?? '';
    tooltipNode.style.left = `${leftPosition}px`;
    tooltipNode.style.top = `${topPosition}px`;

    const bounds = tooltipNode.getBoundingClientRect();
    if (bounds.right > window.innerWidth) {
      tooltipNode.style.left = `${leftPosition - (bounds.right - window.innerWidth)}px`;
    }
  }

  const tooltipNode = createTooltipNode();
  node.append(tooltipNode);

  node.addEventListener(
      'mouseenter',
      () => {
        const bounds = node.getBoundingClientRect();
        tooltipNode.classList.remove(classNameToHide);
        updateTooltipNode(
            tooltipNode,
            textSelector,
            bounds.x + node.clientWidth,
            bounds.y + node.clientHeight / 2
        );
      }
  );

  node.addEventListener('mouseleave', () => tooltipNode.classList.add(classNameToHide));
}
...
// フロートボックス・ドラッグ移動 ----------------------------------------
document.querySelectorAll('.float-box > h2').forEach(obj => makeDraggable(obj));
