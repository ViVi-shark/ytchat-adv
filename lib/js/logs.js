...
window.onload = function(){
  ...
  if(document.getElementById('loglist-area')){
...
function pageSet(set) {
  ...
  document.querySelectorAll('.logs .map-container').forEach(
      node => node.dispatchEvent(new MapResizeEvent())
  );
}
...
function createTableOfContents(){
  ...
  document.querySelectorAll(`[data-headline]`).forEach(obj => {
    ...
    anchor.href = '#'+id;
    anchor.innerHTML = normalizeAnchorText(obj.innerHTML);
    li.append(anchor);
    ul.append(li);
    num++;
  });
...
function normalizeAnchorText(text){
  return text
    .replace(/<br>/gi,' ')
    .replace(/<\/?(.+?)>/g, (match, tagName) => {
      return (['ruby','rt','rp'].includes(tagName)) ? match : '' //ルビタグ以外は消去
    })
...
function bgImageSet(){
  ...
  document.querySelectorAll('.bg-border').forEach(elm => {
    ...
    if(elm.dataset.url){
      let mode = elm.dataset.mode || null;
      bgImages.push( {'url':'url('+elm.dataset.url+')', 'mode': mode, 'title':elm.dataset.title, 'page':page } );
    }
    else {
...
function bgChange(num){
  if(num != currentBg){
    console.log(`bgChange(${num})`);
    {
      ...
      bgBack.dataset.mode = bgFront.dataset.mode ?? '';
    }
    document.querySelector('.bg-front').remove();
    ...
    bgFront.dataset.mode = bgImages[num]['mode'] ?? 'resize';
    document.body.appendChild(bgFront);
    document.getElementById('bg-title').innerHTML = bgImages[num]['title'];
    console.log(`load:${bgImages[num]['url']}`);
    //次の背景画像を読み込んでおく
    if(bgImages[num+1]){
      document.querySelector('.bg-next').style.backgroundImage = bgImages[num+1]['url'];
      console.log(`preload:${bgImages[num+1]['url']}`);
    }
  }
  currentBg = num;
}
...
// 発言者画像 ----------------------------------------
document.querySelectorAll('.logs dl.contains-picture > dt.picture > span').forEach(
    node => node.addEventListener(
        'click',
        () => imgView(window.getComputedStyle(node).backgroundImage.match(/^url\("(.+)"\)$/)[1])
    )
);
