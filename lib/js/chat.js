...
function roomLoad(re){
  ...
  .done( (data) => {
    ...
    if(!romMode){
      ...
      document.getElementById('new-unit-color-value-text').value = nameList[0]['color'];
      $("#new-unit-color-value").spectrum("set", nameList[0]['color']);
    }
    //
...
function logGet(){
  ...
  .done( (data) => {
    ...
    data['logs'].forEach( (value, index, array) => {
      ...
      if(value['system']){
        ...
        // 画像
          ...
          value['info'] = `<img class="insert" loading="lazy" src="${url}" ${ scrollOK ? 'onload="scrollBottom('+value['tab']+')"': '' }>`;
        }
        // 背景
        else if(value['system'].match(/^bg:(.+)$/)){
          const url = RegExp.$1;
          value['info'] += `<img class="insert bg" loading="lazy" src="${url}" ${ scrollOK ? 'onload="scrollBottom('+value['tab']+')"': '' }>`;
        }
        // BGM
        ...
        // レディチェック
          ...
          value['info'] = `<button onclick="checkSubmit(1,'${value['tab']}')">✔準備ＯＫ！</button><button onclick="checkSubmit(0,'${value['tab']}')">×まだ待って</button>`;
        }
        // トピック
      ...
      if(value['comm']){
        ...
        if (loadedLog) {
          const audioNodes = newComm.getElementsByTagName('audio');
          if (audioNodes.length > 0) {
            for (let i = 0; i < lastSoundEffectList.length; i++) {
              lastSoundEffectList[i].pause();
            }
            lastSoundEffectList.splice(0);
          }
          for (let i = 0; i < audioNodes.length; i++) {
            const audioNode = audioNodes[i];
            audioNode.volume = (volumes['chat'] / 100) * (volumes['master'] / 100);
            audioNode.currentTime = 0;
            if (!muteOn) {
              audioNode.play();
              lastSoundEffectList.push(audioNode);
    ...
    // 着信音声
    if(beforeLastnumber && Object.keys(soundFlag).length > 0 && !soundEffectStarted){
...
function topicChange (topicValue){
  const $node = $('#topic-value');

  if (isStoryteller()) {
    storyteller.updateTopicNode($node, topicValue);
    return;
  }

  $node.text('');

  const regexForImgUrl = /^https?\:\/\/.+\.(jpe?g|png|gif|svg)(\?.+)?$/i;

  for (const line of decodeURIComponent((topicValue ?? '')).replace(/\r\n/g, '\n').replace(/\r/g, '\n').replace(/<br(\s*\/)?>/g, '\n').split(/\n/)) {
    if (line.match(regexForImgUrl)) {
      const $imgContainer = $('<p class="img-container" />');
      const timestamp = (new Date()).getTime().toString();
      $imgContainer.attr('data-img-url', line);
      $imgContainer.attr('data-timestamp', timestamp);
      $imgContainer.append($('<span class="url meta" />').text(line));

      const temporaryUrl = (function(originalUrl, timestamp){
        return originalUrl + (originalUrl.indexOf('?') >= 0 ? '&' : '?') + 'dummyTimestamp=' + timestamp;
      })(line, timestamp);

      const $img = $('<img loading="lazy" />').attr('src', temporaryUrl);
      ...
      $node.append($imgContainer);
    } else {
      $node.append($('<p />').text(line));
    }
  }
}
// ステータス表更新 ----------------------------------------
function statusUpdate () {
  for (let name in unitList) {
    ...
    document.getElementById('stt-memo-'+id).setAttribute('data-memo', (unitList[name]['memo'] == null)?'': unitList[name]['memo']);
    ...
    document.getElementById(`stt-unit-${id}`).setAttribute('data-number-of-items', viewCount);
  }
...
function checkSubmit(check,target){
  let name  = nameList[document.getElementById('form-name').value]['name'];
  const color  = nameList[document.getElementById('form-name').value]['color'];
  commSend( (check?'@check':'@uncheck'), target, name, color );
}
