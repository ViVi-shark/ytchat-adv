...
function makeFuncToCreateList (sign) {
  const re = new RegExp(`^\\s*[${sign}]\\s*`, 'gm');
  return text => '<ul>' + text.replace(re, '<li>').replace(/\n/, '') + '</ul>';
}
...
function statusUpdate () {
  for (let name in unitList) {
    ...
    if (memoNode != null) {
      memoNode.setAttribute('data-memo', (unitList[name]['memo'] == null) ? '' : unitList[name]['memo']);
    }
    ...
    document.getElementById(`stt-unit-${id}`).setAttribute('data-number-of-items', viewCount.toString());
  }
...
// 送信 ----------------------------------------
function commSend(comm,tab,name,color,address,bcdice,onCompleted=null,onError=null){
    ...
    if(bcdiceAPI){
      ...
      if(hit){
        ...
        .then(data => {
          ...
          commSend(comm, target, name, color, address, bcdice, resolution, rejection);
        })
        .catch(error => {
          console.error(error);
          commSend(comm, target, name, color, address, bcdice, resolution, rejection);
      ...
      else { commSend(comm, target, name, color, address, null, resolution, rejection); }
    }
    // 通常送信処理
    else {
      commSend(comm, target, name, color, address, null, resolution, rejection);
    }
  });
}
// タブコマンドチェック ----------------------------------------
