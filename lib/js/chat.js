    ...
    data['logs'].forEach( (value, index, array) => {
      ...
      if(value['system']){
        ...
        // ランダム表の定義
          ...
          value['comm'] += `（実行コマンド： ${tagConvert(`\`1$${m[1]}\``)} ）`;
        }
        // ユニット状態
          ...
          if (value['info'] != null && value['info'] !== '') {
            ...
            if (info['differences']) {
              if (info['differences'].length > 0) {
                ...
                const differenceCount = differenceList.querySelectorAll('.state').length;

                if (differenceCount >= 2) {
                  // 2 つ以上の状態の変化があれば折り畳めるように.
                  value['info'] = `<details open><summary><span class="title">${differenceCount} states changed.</span></summary>${value['info']}</details>`;
                }
              } else {
      ...
      if(value['comm']){
        ...
        if (value['system'] == null || value['system'].startsWith('unit:')) {
          if (value['name'] != null && value['name'] in unitList) {
            const mentionedStates =
                findMentionedStates(value['comm'])
                    .filter(x => x.source === '#self')
                    .filter(x => x.duration != null);

            if (0 < mentionedStates.length && mentionedStates.length <= 7) {
              const dd = document.createElement('dd');
              dd.classList.add('commands');
              dd.dataset.commentId = newComm.id;

              for (const state of mentionedStates) {
                const button = document.createElement('button');
                button.classList.add('to-add-state');
                button.textContent = `状態を追加: ${state.name}`;
                dd.appendChild(button);

                button.addEventListener(
                    'click',
                    ((unitName, state, button) => {
                      return () => {
                        button.disabled = true;

                        const operation = new StateOperationToAdd();
                        operation.stateName = state.name;
                        operation.stateIcon.category = state.icon.category;
                        operation.stateIcon.direction = state.icon.direction;
                        operation.stateSource = state.source;
                        operation.stateDuration = state.duration;
                        operation.stateDescription = state.description;

                        for (const [k, v] of Object.entries(state.additionalFields ?? {})) {
                          operation.additionalFields.set(k, v);
                        }

                        operation.targets = [unitName];

                        if (operation.isValid) {
                          const command = operation.buildCommand();
                          sendStateCommand(command).then(() => button.remove());
                        } else {
                          alert(operation.findErrorMessage());
                        }
                      };
                    })(value['name'], state, button)
                );
              }

              targetDl.appendChild(dd);
              targetDl.style.gridTemplateRows = `max-content repeat(${targetDl.querySelectorAll('dd').length}, max-content) 1fr`;
            }
          }
        }
      ...
      if(value['info']){
        ...
        if(value['system']){
          ...
          if(value['system'].match(/^(topic|choice(?!:table)|deck)/)) { value['info'] = tagConvert(value['info']) }
        ...
        if (newInfo.classList.contains('dice')) {
          const rows = newInfo.innerHTML.split('<br>');

          newInfo.innerHTML = rows.map(x => `<span class="row">${x}</span>`).join('\n');

          // ５行以上のダイスロール結果は折り畳めるようにする.
          if (rows.length >= 5) {
            newInfo.innerHTML = `<details open><summary><span class="code">${newInfo.dataset.code}</span><span class="row-count">[${rows.length} rows]</span></summary>${newInfo.innerHTML}</details>`;
          }
        } else if (newInfo.classList.contains('unit') && value['info'].includes('<a href=')) {
          // ユニット追加ログを折り畳めるように.
          newInfo.innerHTML = `<details open><summary><span class="title">ユニット追加：${newInfo.querySelector('a[href]').textContent.trim()}</span></summary>${newInfo.innerHTML}</details>`;
        } else if (value['system'] === 'topic') {
          // トピック変更ログを折り畳めるように.
          newInfo.innerHTML = `<details open><summary><span class="title">...</span></summary>${newInfo.innerHTML}</details>`;
        }
...
/**
 * @typedef TagConvertOptions
 * @property {boolean} [noStandardTooltip]
 */
/**
 * @param {string} comm
 * @param {null|TagConvertOptions} options
 * @return {string}
 */
function tagConvert (comm, options = null){
  /**
   * @param {string} text
   * @return {string}
   */
  function toCharacterReference(text) {
    const destination = [];
    let i = 0;
    while (i < text.length) {
      const m = text.substring(i).match(/^&[a-z0-9]+;/);
      if (m != null) {
        destination.push(m[0]);
        i += m[0].length;
      } else {
        destination.push(`&#${text.charCodeAt(i)};`);
        i++;
      }
    }
  ...
  comm = comm.replace(/`([^`]+?)`/g, (_, x) => `&lt;snippet&gt;${toCharacterReference(x)}&lt;/snippet&gt;`);
  ...
  while(comm && comm !== str){
    ...
    comm = comm
      ...
      .replace(/\n?&lt;map\s*\/&gt;\n?/i, '<div class="embedded-map not-initialized"></div>')
  }
  comm = comm.replaceAll(/(<\/ul>)\n/g, '$1');
  if (!options?.noStandardTooltip) {
    (tooltips ?? []).forEach(h => {
      Object.keys(h).forEach(key => {
        if (!key) return;
        if(comm.match(key)){
          comm = comm.replace(new RegExp(key,'g'), (term) => {
            tooltipSet.push( { 'term':term, 'desc':resolveDataStructure(h[key]) } );
            return `<!tooltip#${tooltipSet.length}>`;
          });
        }
      });
...
function topicChange (topicValue){
  ...
  topicElement.querySelectorAll('.embedded-map.not-initialized').forEach(
      mapElement => {
        setupAnotherMapView(mapElement);
        mapElement.classList.remove('not-initialized');
      }
  );
}
...
function updateMap(encodedSourceText) {
  sendEventToMap(() => new MapUpdateEvent(encodedSourceText));
}

function refreshMap() {
  sendEventToMap(() => new MapRefreshEvent());
}

/**
 * @callback sendEventToMap~eventSupplier
 * @return {Event}
 */
/**
 * @param {sendEventToMap~eventSupplier} eventSupplier
 */
function sendEventToMap(eventSupplier) {
  document.querySelectorAll('.map-window').forEach(
      node => node.dispatchEvent(eventSupplier())
  );
}
  ...
  for (let name in unitList) {
    ...
    if (gameSystem === 'sw2') {
      ...
      const groupingRules = [
        {
          name: '赤',
          re: /^(?:MC)?赤([BＢ]|[AＡ]|[SＳ]{1,2})$/
        },
        {
          name: '緑',
          re: /^(?:MC)?緑([BＢ]|[AＡ]|[SＳ]{1,2})$/
        },
        {
          name: '黒',
          re: /^(?:MC)?黒([BＢ]|[AＡ]|[SＳ]{1,2})$/
        },
        {
          name: '白',
          re: /^(?:MC)?白([BＢ]|[AＡ]|[SＳ]{1,2})$/
        },
        {
          name: '金',
          re: /^(?:MC)?金([BＢ]|[AＡ]|[SＳ]{1,2})$/
        },
        {
          name: '陽光',
          re: /^陽光([123①②③ⅠⅡⅢ])$/,
        ...
        {
          name: '拡張素材',
          re: /^((?:悪魔の)?血|(?:悪魔の)血晶|(?:(?:大悪魔の)?血晶|大悪魔|大血晶)|(?:アビス)?シャード)$/,
        },
      ];
      ...
      for (const rule of groupingRules) {
        ...
        for (let i = 0; i < status.length; i++) {
          ...
          groupItems.push({name: m[1], originalName: s, value: unitList[name]['status'][s] ?? ''});
          status[i] = null;
        }
        ...
        groups.push({name: rule.name, items: groupItems});
      }
    ...
    for (let i in status) {
      ...
      let beforeRawValue = null;
      if(obj){
        beforeRawValue = obj.dataset.rawValue;
       ...
       * @return {string}
       */
      function commify(source) {
        return source.toString().replace(/([0-9]{1,3})(?=(?:[0-9]{3})+(?![0-9]))/g, '$1,');
      }
      const currentRawValue = value?.toString() ?? '';
      valueContainer.dataset.rawValue = currentRawValue;
      if(value && value.match(/&nbsp;|\s/)){
        ...
        function makeHtml(now, max) {
          ...
          function valueToHtml(value) {
              return value > 0
                  ? commify(value)
                  : value < 0
                      ? `<span class="minus">${commify(value)}</span>`
                      : `<span class="zero">${value}</span>`;
          }

          return `${valueToHtml(now)}/${valueToHtml(max)}`;
        }
        ...
        const signal = (per >= 75) ? 'safe'
                     ...
                     : 'knockdown';
        if(per < 0) { per = 0; }
        if(valueNum.innerHTML !== makeHtml(now, max)){
          valueNum.innerHTML = makeHtml(now, max);
          valueGauge.classList.remove('none');
          gaugeUpdate(valueGauge, valueGaugeNow, per, signal, targetNames != null && currentRawValue === beforeRawValue);
        }

        valueNum.dataset.rawNow = now.toString();
        valueNum.dataset.rawMax = max.toString();
        ...
        if(valueNum.innerHTML !== value){
          ...
          valueNum.dataset.rawNow = value.toString();
          valueGauge.classList.remove('none');
        ...
        if ((isNumeric = /^-?\d+$/.test(value))) {
          ...
          if (now < 0) {
            valueNum.innerHTML = `<span class="minus">${commify(now)}</span>`;
          } else if (now === 0) {
            valueNum.innerHTML = `<span class="zero">${now}</span>`;
          } else {
            valueNum.innerHTML = commify(now);
          }

          valueNum.dataset.rawNow = now.toString();
        }
      }
      valueNum.setAttribute('title', value?.toString() ?? '');
      // 値が入ってない項目は非表示
    ...
    for (const group of groups) {
      const groupElement =
          document.querySelector(`#stt-unit-${id} [data-group-name="${group.name}"]`) ??
          ((unitId, groupName) => {
            const e = document.createElement('li');
            e.classList.add('group');
            e.dataset.groupName = groupName;
            ...
            const ul = document.createElement('ul');
            ul.classList.add('group-items');
            e.appendChild(ul);

            const unitDetails = document.querySelector(`#stt-unit-${unitId}  ul.details`);
            unitDetails.insertBefore(e, unitDetails.querySelector('.states') ?? null);

            return e;
          })(id, group.name);

      groupElement.dataset.rowIndex = (rowCount + 1).toString();

      group.items.forEach(
          groupItem => {
            const itemElement =
                groupElement.querySelector(`.group-items > [data-item-name="${groupItem.name}"]`) ??
                (item => {
                  const e = document.createElement('li');
                  e.classList.add('group-item');
                  e.dataset.itemName = item.name;
                  ...
                  e.dataset.stt = item.originalName ?? item.name;
                  e.addEventListener('click', openManipulation);

                  return e;
                })(groupItem);

            if (itemElement.parentNode == null) {
              const list = groupElement.querySelector('ul.group-items');
            ...
            const itemValueElement = itemElement.querySelector('.item-value');
            itemValueElement.textContent = groupItem.value;

            if (/^-?\d+$/.test(groupItem.value)) {
              const now = parseInt(groupItem.value);
              const manipulationTokens =
                  (now !== 0 ? ['=0'] : [])
                      .concat(makeRange(1, Math.max(20, Math.ceil(now / 2))).slice(0, 30))
                      .concat(makeRange(-1, Math.min(-20, -Math.ceil(now / 2))).slice(0, 30));

              itemElement.dataset.manipulationAllowed = 'yes';
              itemElement.dataset.manipulationTokens =
                  manipulationTokens.map(x => typeof x === 'number' && x > 0 ? `+${x}` : x).join(',');
            } else {
              itemElement.dataset.manipulationAllowed = 'no';
              itemElement.dataset.manipulationTokens = '';
            }
          }
      );
...
function gaugeUpdate(maxObj,nowObj,per,signal, immediately = false){
  if (immediately) {
