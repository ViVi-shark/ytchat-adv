/**
 * @typedef StateTemplate_Icon
 * @property {string} category
 * @property {string} direction
 */

/**
 * @typedef StateTemplate_AdditionalFields
 * @property {string} [達成値]
 */

/**
 * @typedef StateTemplate
 * @property {string} name
 * @property {StateTemplate_Icon} icon
 * @property {string|null} duration
 * @property {string|null} [description]
 * @property {string|null} [source]
 * @property {StateTemplate_AdditionalFields} [additionalFields]
 */

/**
 * @typedef CategoryStateTemplateCollection
 * @property {string} categoryName
 * @property {StateTemplate[]} states
 */

/**
 * @typedef GameStateTemplateCollection
 * @property {string} gameName
 * @property {CategoryStateTemplateCollection[]} categories
 */
...
function createUnitStateDurationNode(state) {
    ...
    const durationNumberText = state.duration.number?.toString() ?? '';
    const durationUnitText = state.duration.unit;

    {
        const durationNumberNode = document.createElement('span');
        durationNumberNode.classList.add('duration-number');
        durationNumberNode.textContent = durationNumberText;
...

    {
        ...
        durationUnitNode.textContent = durationUnitText;
    ...
    durationNode.setAttribute(
        'title',
        `${durationNumberText}${durationUnitText}`
    );
...
function updateUnitState(unitId) {
    ...
    for (const state of loadUnitStates(unitName)) {
        ...
        if (stateNode.parentNode == null) {
            ...
            const list =
                unitNode.querySelector('ul.details > .states .state-list') ??
                (unitNode => {
                    const statesNode = document.createElement('li');
                    statesNode.classList.add('states');
                    unitNode.querySelector('ul.details').appendChild(statesNode);

                    const details = document.createElement('details');
                    details.open = true;
                    statesNode.appendChild(details);
    ...
    {
        const summary = unitNode.querySelector('ul.details > .states > details > summary > .content');

        if (summary != null) {
            summary.textContent = `${stateCount}件の状態`;
        }
    }
...
class StateModificationSettings_TargetFilter {
     ...
     */
    condition(additionalFieldName, condition) {
        if (condition != null && condition !== '') {
            this.#additionalFields[additionalFieldName] = condition.toString();
        } else {
            delete this.#additionalFields[additionalFieldName];
        }
    }
    ...
    findErrorMessage() {
        if (
            ...
            !Object.values(this.#additionalFields).some(x => x != null)
        ) {
            return "すくなくともひとつの条件を指定してください";
        }
...
(() => {
                    ...
                    <div class="panes">
                        ...
                        <div class="state-settings-area">
                            ...
                            <select id="state-icon-settings" data-default-value="汎用">
                                ...
                                <optgroup label="FFXIVTTRPG">
                                    <option value="FFXIVTTRPG/生還">生還</option>
                                    <option value="FFXIVTTRPG/スタン">スタン</option>
                                    <option value="FFXIVTTRPG/衰弱">衰弱</option>
                                    <option value="FFXIVTTRPG/ランパート">ランパート</option>
                                    <option value="FFXIVTTRPG/リプライザル">リプライザル</option>
                                    <option value="FFXIVTTRPG/バーサク">バーサク</option>
                                    <option value="FFXIVTTRPG/ルーシッドドリーム">ルーシッドドリーム</option>
                                    <option value="FFXIVTTRPG/エアロ">エアロ</option>
                                    <option value="FFXIVTTRPG/アドル">アドル</option>
                                    <option value="FFXIVTTRPG/アストラルファイア">アストラルファイア</option>
                                    <option value="FFXIVTTRPG/アンブラルブリザード">アンブラルブリザード</option>
                                    <option value="FFXIVTTRPG/サンダー系">サンダー系</option>
                                    <option value="FFXIVTTRPG/サンダー系Proc">サンダー系Proc</option>
                                </optgroup>
                            </select>
                        ...
                        <div class="target-selector-area">
                            ...
                            <div class="state-selector-area">
                                ...
                                <div class="filter-area">
                                    ...
                                    <select id="state-modification-target-by-state-name" class="value"></select>
                                    <label for="state-modification-target-by-source">状態の発生源</label>
                                    <select id="state-modification-target-by-source" class="value"></select>
                                    <label for="state-modification-target-by-holder">状態の保有者</label>
                                    <select id="state-modification-target-by-holder" class="value"></select>
                                    <label for="state-modification-target-by-duration-unit">時間の単位</label>
                                    <select id="state-modification-target-by-duration-unit" class="value"></select>
                                </div>
        ...
        switch (gameCode) {
            case 'sw2': {
...

                {
                    ...
                    modeSelector.setAttribute('id', 'state-modification-achievement-mode');
                    modeSelector.classList.add('mode');
                    modeSelector.append(
                        ...(() => {
                            const option = document.createElement('option');
                            option.textContent = "以下";
                    ...
                    const input = document.createElement('input');
                    input.setAttribute('id', 'state-modification-target-by-achievement-value');
                    input.setAttribute('type', 'number');
                    input.classList.add('additional');
                    input.dataset.fieldName = "達成値";
                    input.dataset.modeSelectorId = modeSelector.getAttribute('id');

                    const label = document.createElement('label');
                    label.textContent = input.dataset.fieldName;
                    label.setAttribute('for', input.getAttribute('id'));

                    window.querySelector('.state-selector-area .filter-area').append(
                        label,
                        input,
                        modeSelector
                    );
                            ...
                            node.addEventListener(
                                ...
                                () => {
...

                                    {
                                        ...
                                        window.querySelectorAll('.state-settings-area .additional-fields input[name]').forEach(
                                            x => additionalControls[x.name] = x
                                        );

                                        for (const [key, value] of Object.entries(state.additionalFields)) {
                                            if (additionalControls[key] != null) {
                                                setValue(additionalControls[key], value);
                                            }
                                        }
                                    }
                            ...
                            category => {
                                ...
                                category.states.forEach(
                                    stateSettings => {
                                        ...
                                        if (stateSettings.additionalFields != null) {
                                            Object.assign(additionalFields, stateSettings.additionalFields);
                                        }
...

    {
        ...
        const onUpdated = () => {
            ...
            for (const additionalControl of window.querySelectorAll('.state-selector-area .filter-area :is(input, select).additional')) {
                const fieldName = additionalControl.dataset.fieldName;
                const value = additionalControl.value.trim();
                const mode =
                    additionalControl.hasAttribute('data-mode-selector-id')
                        ? window.querySelector(`#${additionalControl.dataset.modeSelectorId}`).value.trim()
                        : null;

                const condition =
                    mode != null && value != null && value !== ''
                        ? `${mode}${value}`
                        : value !== ''
                            ? value
                            : null;

                operationToModify.targets.condition(fieldName, condition);
                    ...
                    switch (fieldName) {
                        case "達成値":
                            additionalFilters.push(
                                ((fieldName, condition) => {
                                    return /** @param {HTMLElement} stateElement */stateElement => {
                                        const attributeName = `data-${fieldName}`;

                                        return stateElement.hasAttribute(attributeName) &&
                                            parseInt(stateElement.getAttribute(attributeName)) <= parseInt(condition);
                                    };
                                })(fieldName, value)
                            );
                            break;
            ...
            if (
                ...
                targetList.querySelectorAll(query).forEach(
                    x => {
                        if (
                            additionalFilters.length > 0 &&
                            additionalFilters.some(filter => !filter(x))
                        ) {
                            return;
                        }

                        x.classList.add(classNameToSelect)
                    }
                );
            }
        ...
        window.querySelectorAll('.state-selector-area .filter-area :is(input, select).additional').forEach(
            additionalControl => {
                additionalControl.addEventListener('input', onUpdated);
                additionalControl.addEventListener('change', onUpdated);
            }
        );
            ...
            unitName => {
                ...
                (statesByUnitName[unitName] ?? []).forEach(
                   /** @param {UnitContinuationState} state */ state => {
                       ...
                       for (const [key, value] of Object.entries(state.additionalFields)) {
                           stateNode.setAttribute(`data-${key}`, value);
                       }
 ...
 */
function findMentionedStates(comment) {
    const normalizedComment = normalizeStateName(comment);

    /** @var {StateTemplate[]} */
    const mentionedStates = [];

    for (const game of stateTemplates) {
        for (const category of game.categories) {
            for (const state of category.states) {
                if (
                    [...enumerateStateNamePart(normalizeStateName(state.name))].some(
                        stateNamePart => normalizedComment.includes(stateNamePart)
                    )
                ) {
