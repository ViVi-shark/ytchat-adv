            ...
            buttonToSubmit.addEventListener(
                ...
                () => {
                    const temporaryNode = makeTemporaryTextAreaNode(
                        encodeMapSourceText(textarea.value)
                    );
 ...
 */
function setupAnotherMapView(viewElement) {
    if (globalMap === null) {
        throw new Error('Map is not supported in this room.');
    }

    const renderer = globalMap.renderTo(viewElement);

    {
        const sheetArea = document.getElementById('sheet-area');

        sheetArea.addEventListener('sheet-open', () => renderer.resize());
        sheetArea.addEventListener('sheet-close', () => renderer.resize());
    }
}
