...
($ => {
    ...
    {
        const $editTopic = $('#edit-topic');
        $editTopic.addClass('storyteller-keyword-area');

        const $buttonToInsertCheck = $('<button />').text('チェックマークを挿入');

        $editTopic.append(
            $('<div class="bottom-area" />')
                .append($buttonToInsertCheck)
        );

        {
            const $textarea = $('#edit-topic textarea');

            /** @var {HTMLTextAreaElement} */
            const textarea = $textarea[0];

            $buttonToInsertCheck.on(
                'click',
                () => {
                    if ($buttonToInsertCheck.attr('disabled') != null) {
                        return;
                    }
            ...
            $textarea.focus(() => $buttonToInsertCheck.removeAttr('disabled'));

            $textarea.blur(
                e => {
                    if (e.relatedTarget === $buttonToInsertCheck[0]) {
                        return;
                    }

                    $buttonToInsertCheck.attr('disabled', '');
                }
            ).trigger('blur');
        }
    }
